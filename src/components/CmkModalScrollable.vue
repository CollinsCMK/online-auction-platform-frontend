<template>
    <div ref="target" class="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-100 bg-white dark:bg-gray-900 w-[95%] md:w-[80%] lg:w-1/2 xl:w-[45%] rounded-lg shadow-2xl outline-hidden max-h-[90vh] sm:max-h-[80vh] overflow-hidden transition-all duration-300 ease-in-out scale-100 hover:scale-[1.02]" :class="myClass">
        <div class="bg-linear-to-r from-flex-purple to-purple-600 p-4 sm:p-5 text-white flex items-center justify-between">
            <h4 class="font-bold text-lg sm:text-xl tracking-wide">{{ title }}</h4>

            <div @click="$emit('cancel')" class="flex items-center justify-center w-7 h-7 rounded-full border border-white/50 cursor-pointer hover:bg-white/20 transition-colors duration-200">
        ...
        <div class="backdrop-blur-xs bg-white/90 dark:bg-gray-900/90">
            <form @submit.prevent="$emit('submitForm')" class="p-4 sm:p-6 md:p-8">
                ...
                <div class="flex flex-col sm:flex-row justify-end items-center gap-4 sm:gap-6 mt-6 border-t pt-6 dark:border-gray-700">
                    <CmkButton @click="$emit('cancel')" type="button" 
