<template>
  <div class="p-2 sm:p-4 dark:bg-gray-900 dark:text-white transition-colors duration-200">
    <div
      class="rounded-lg pb-2 shadow-[0px_0px_2px_1px_#aaa] backdrop-blur-xs bg-white/80 dark:bg-gray-900/80"
    >
          ...
          <div class="flex gap-2 items-center">
            <div v-if="isRefresh">
              ...
              <CmkButton
                ...
                type="button"
                @click="$emit('refresh')"
                class="flex items-center gap-2 border hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg px-4 py-2 transition-all duration-300 transform hover:scale-105"
                ><IoIosRefresh class="animate-spin" />Refresh</CmkButton
              >
            </div>
            ...
            <CmkButton
              ...
              @click="$emit('create')"
              class="flex items-center gap-2 border hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg px-4 py-2 transition-all duration-300 transform hover:scale-105"
              ><IoIosCreate class="animate-bounce" />Create</CmkButton
            >
          </div>
            ...
            <thead>
              <tr class="bg-linear-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-900">
                <th
                  ...
                  :class="{
                    'sm:sticky left-0 z-10 sm:bg-white dark:sm:bg-gray-900':
                      column.props.fixed === 'left',
                    'sm:sticky right-0 z-10 sm:bg-white dark:sm:bg-gray-900':
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    <div class="flex flex-col text-green-600 dark:text-green-400">
                      <FaArrowUpShortWide
                        v-if="sortedColumn === column.props.field && sortOrder === 'asc'"
                      />
                      <FaArrowDownShortWide
                        v-else-if="sortedColumn === column.props.field && sortOrder === 'desc'"
                      />
                    </div>
                  </div>
                </th>
            ...
            <tbody v-else-if="handleSearch(paginatedItems).length > 0">
              ...
              >
                <td
                  ...
                  :class="{
                    'sm:sticky left-0 z-10 sm:bg-white dark:sm:bg-gray-900 sm:even:bg-gray-50 dark:sm:even:bg-gray-800':
                      column.props.fixed === 'left',
                    'sm:sticky right-0 z-10 sm:bg-white dark:sm:bg-gray-900 sm:even:bg-gray-50 dark:sm:even:bg-gray-800':
                      column.props.fixed === 'right',
                  }"
                  :style="{ width: column.props.width }"
                >
                  ...
                  <component v-else :is="column.children.default" v-bind="{ item }" />
                </td>
              </tr>
            </tbody>
            <tbody v-else>
      ...
      <div v-if="isTableFooter" class="flex flex-col sm:flex-row items-center justify-center gap-6">
        <div class="hidden sm:block">
          <p class="text-gray-600 dark:text-gray-400 font-medium">
            Total Records: {{ handleSearch(items).length }}
          </p>
        </div>
...
const totalPages = computed(() => {
  const pages = Math.ceil(handleSearch(items.value).length / itemsPerPage.value)
